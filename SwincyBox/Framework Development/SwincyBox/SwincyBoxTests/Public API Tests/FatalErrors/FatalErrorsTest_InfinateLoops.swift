...
class FatalErrorsTest_InfinateLoops: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        box.register(life: .permanent) { r in
            SafetyTestCar(driver: r.resolve() as CrashTestDummy)
        }
        box.register(CrashTestDummy.self, life: .permanent) { resolver in
            CrashTestDummy(car: resolver.resolve())
        }
    }
    ...
    func testThatCarCircularDependencyExists() {
        do {
            ...
            XCTFail("Failed to throw a fatalError() when an infinate loop is detected")
        }
        catch {
